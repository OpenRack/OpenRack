...
namespace cYo.Projects.ComicRack.Viewer.Dialogs
{
  public class ShowErrorsDialog : Form
  {
    ...
    private void AddError(ShowErrorsDialog.ErrorItem ei) => this.lvErrors.Items.Add(ei.Item).SubItems.Add(ei.Message);
    ...
    public static ShowErrorsDialog.ErrorItem ComicExporterConverter(ComicExporter ce) => new ShowErrorsDialog.ErrorItem()
    {
      Item = ce.ComicBooks[0].FileNameWithExtension,
      Message = ce.LastError
    };

    public static ShowErrorsDialog.ErrorItem DeviceSyncErrorConverter(DeviceSyncError ce) => new ShowErrorsDialog.ErrorItem()
    {
      Item = ce.Name,
      Message = ce.Message
    };
