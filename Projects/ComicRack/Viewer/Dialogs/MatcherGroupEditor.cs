...
namespace cYo.Projects.ComicRack.Viewer.Dialogs
{
  public class MatcherGroupEditor : UserControl, IMatcherEditor
  {
    ...
    private void btEdit_Click(object sender, EventArgs e) => this.cmEdit.Show((Control) this.btEdit, new System.Drawing.Point(this.btEdit.Width, this.btEdit.Height), ToolStripDropDownDirection.BelowLeft);
    ...
    private void cbMatchMode_SelectedIndexChanged(object sender, EventArgs e) => this.currentComicBookMatcher.MatcherMode = this.cbMatchMode.SelectedIndex == 0 ? MatcherMode.And : MatcherMode.Or;

    private void chkNot_CheckedChanged(object sender, EventArgs e) => this.currentComicBookMatcher.Not = this.chkNot.Checked;
    ...
    public Control CreateMatchPanel(ComicBookValueMatcher matcher, int width) => (Control) new MatcherEditor(this.currentComicBookMatcher.Matchers, matcher, this.level, width);

    public Control CreateGroupMatchPanel(ComicBookGroupMatcher matcher, int width) => (Control) new MatcherGroupEditor(this.currentComicBookMatcher.Matchers, matcher, this.level + 1, width);

    public string Description => this.matcherControls.GetControls<MatcherEditor>().Select<MatcherEditor, string>((Func<MatcherEditor, string>) (me => me.Description.Ellipsis(20, "..."))).ToListString(", ");
    ...
    public void DeleteRuleOrGroup() => this.matchers.Remove((ComicBookMatcher) this.currentComicBookMatcher);

    public void CopyClipboard() => Clipboard.SetData("ComicBookMatcher", (object) this.currentComicBookMatcher);
    ...
    public void MoveUp() => this.matchers.MoveRelative(this.matchers.IndexOf((ComicBookMatcher) this.currentComicBookMatcher), -1);

    public void MoveDown() => this.matchers.MoveRelative(this.matchers.IndexOf((ComicBookMatcher) this.currentComicBookMatcher), 1);
