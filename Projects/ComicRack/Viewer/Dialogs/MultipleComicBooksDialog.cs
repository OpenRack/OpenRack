...
namespace cYo.Projects.ComicRack.Viewer.Dialogs
{
  public class MultipleComicBooksDialog : Form
  {
      ...
      foreach (CheckBox control in this.GetControls<CheckBox>())
      {
        if (control.CheckState != CheckState.Unchecked)
        {
          ...
          if (tag1.Enabled)
          {
            ...
            PropertyInfo member = tag2.StartsWith("{") ? (PropertyInfo) null : typeof (ComicBook).GetProperty(tag2);
            ...
            else if (member.PropertyType == typeof (YesNo))
              storeInfo.Value = (object) EditControlUtility.GetYesNo(text);
            else if (member.PropertyType == typeof (MangaYesNo))
            ...
            else if (member.PropertyType == typeof (int))
            {
              ...
              if (!int.TryParse(text, out result) || result < 0)
                result = member.DefaultValue<int>(-1);
            ...
            else if (member.PropertyType == typeof (float))
            {
              ...
              if (!float.TryParse(text, out result))
                result = member.DefaultValue<float>();
              storeInfo.Value = (object) result;
            }
    ...
    private void SetSimpleText(Control c, CheckBox chk, string propName)
    {
      ...
      PropertyInfo propertyInfo = flag1 ? (PropertyInfo) null : typeof (ComicBook).GetProperty(propName);
      foreach (ComicBook book in this.books)
      {
        object objA = propertyInfo == (PropertyInfo) null ? (object) book.GetStringPropertyValue(propName) : propertyInfo.GetValue((object) book, (object[]) null);
        if (objB == null)
          objB = objA;
