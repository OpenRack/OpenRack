...
namespace cYo.Projects.ComicRack.Viewer.Dialogs
{
  public class SmartListQueryDialog : Form, ISmartListDialog
  {
    ...
    private void OnInsertQuery(object sender, EventArgs e) => this.rtfQuery.SelectedText = "[" + (((ToolStripItem) sender).Tag as IComicBookValueMatcher).DescriptionNeutral + "]";

    private void OnInsertField(object sender, EventArgs e) => this.rtfQuery.SelectedText = "{" + (string) (sender as ToolStripMenuItem).Tag + "}";
    ...
    private void rtfQuery_SelectionChanged(object sender, EventArgs e) => this.UpdatePositionDisplay();
    ...
    private void AddUndo() => this.AddUndo(this.rtfQuery.Rtf, this.rtfQuery.SelectionStart, this.rtfQuery.SelectionLength);
    ...
    private Tokenizer.Token GetCurrentToken(int n) => ComicSmartListItem.TokenizeQuery(this.rtfQuery.Text).GetAll().FirstOrDefault<Tokenizer.Token>((Func<Tokenizer.Token, bool>) (t => n >= t.Index && n <= t.Index + t.Length));
