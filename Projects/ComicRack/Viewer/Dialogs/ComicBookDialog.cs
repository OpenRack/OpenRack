...
namespace cYo.Projects.ComicRack.Viewer.Dialogs
{
  public class ComicBookDialog : Form
  {
    ...
    private void PagesViewItemActivate(object sender, EventArgs e) => this.tabControl.SelectedTab = this.tabColors;
    ...
    private static void SetCoverThumbnailImage(IBitmapDisplayControl iv, ComicBook cb) => ComicBookDialog.SetThumbnailImage(iv, cb, cb.FrontCoverPageIndex);
    ...
    private void SetCurrentColorAdjustment(BitmapViewer iv, Color whitePoint) => iv.ColorAdjustment = new BitmapAdjustment((float) this.tbSaturation.Value / 100f, (float) this.tbBrightness.Value / 100f, (float) this.tbContrast.Value / 100f, (float) this.tbGamma.Value / 100f, whitePoint, sharpen: this.tbSharpening.Value);

    private void SetCurrentColorAdjustment(BitmapViewer iv) => this.SetCurrentColorAdjustment(iv, iv.ColorAdjustment.WhitePointColor);
    ...
    private void chkShowColorControls_CheckedChanged(object sender, EventArgs e) => this.panelImageControls.Visible = this.chkShowImageControls.Checked;
    ...
    private void ColorAdjustment_Scroll(object sender, EventArgs e) => this.SetCurrentColorAdjustment(this.pageViewer);
    ...
    private void btResetPages_ShowContextMenu(object sender, EventArgs e) => this.miOrderByName.Enabled = this.miOrderByNameNumeric.Enabled = this.pagesView.Book != null && this.pagesView.Book.IsIndexRetrievalCompleted;
    ...
    private void cbEnableShadowValues_SelectedIndexChanged(object sender, EventArgs e) => this.SetPromptTexts(this.current, EditControlUtility.GetYesNo(this.cbEnableProposed.Text) == YesNo.Yes);

    private void btPageViews_Click(object sender, EventArgs e) => this.pagesView.ItemView.GetViewMenu().Show((Control) this.btPageView, new System.Drawing.Point(this.btPageView.Width, this.btPageView.Height), ToolStripDropDownDirection.BelowLeft);
    ...
    private void coverThumbnail_Click(object sender, EventArgs e) => this.coverThumbnail.ThreeD = !this.coverThumbnail.ThreeD;

    private void linkLabel_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e) => Program.StartDocument(this.linkLabel.Text);
    ...
    private void ComicBookDialog_DragOver(object sender, DragEventArgs e) => e.Effect = e.Data.GetDataPresent(DataFormats.FileDrop) ? DragDropEffects.Copy : DragDropEffects.None;
    ...
    private void miResetThumbnail_Click(object sender, EventArgs e) => this.CustomThumbnailKey = (string) null;
    ...
    private void InitializeScriptButton()
    {
      if (ComicBookDialog.scriptEngine == null)
        return;
      foreach (cYo.Projects.ComicRack.Plugins.Command command in ComicBookDialog.scriptEngine.GetCommands("Editor"))
      {
        if (this.btScript.ContextMenuStrip == null)
    ...
    private void DropDownMenuItemClick(object sender, EventArgs e)
    {
      ...
      this.ExecuteScript(toolStripMenuItem.Text, toolStripMenuItem.Tag as cYo.Projects.ComicRack.Plugins.Command);
    }

    private void ScriptButtonClick(object sender, EventArgs e)
    {
      Control control = (Control) sender;
      this.ExecuteScript(control.Text, control.Tag as cYo.Projects.ComicRack.Plugins.Command);
    }

    private void ExecuteScript(string name, cYo.Projects.ComicRack.Plugins.Command cmd)
    {
      try
      {
    ...
    public static bool Show(IWin32Window parent, ComicBook comicBook, ComicBook[] books) => ComicBookDialog.Show(parent, comicBook, books, (Func<ComicBook, bool>) null);

    public static bool Show(IWin32Window parent, ComicBook comicBook) => ComicBookDialog.Show(parent, comicBook, (ComicBook[]) null);
