...
namespace cYo.Projects.ComicRack.Viewer.Dialogs
{
  public class PreferencesDialog : Form
  {
    ...
    private void chkLibraryGauges_CheckedChanged(object sender, EventArgs e) => this.chkLibraryGaugesNew.Enabled = this.chkLibraryGaugesUnread.Enabled = this.chkLibraryGaugesTotal.Enabled = this.chkLibraryGaugesNumeric.Enabled = this.chkLibraryGauges.Checked;
    ...
    private void lbPaths_DragOver(object sender, DragEventArgs e) => e.Effect = e.Data.GetDataPresent(DataFormats.FileDrop) ? DragDropEffects.Copy : DragDropEffects.None;

    private void lvScripts_ItemChecked(object sender, ItemCheckedEventArgs e)
    {
      if (!(e.Item.Tag is cYo.Projects.ComicRack.Plugins.Command tag))
        return;
    ...
    {
      cYo.Projects.ComicRack.Plugins.Command command = this.lvScripts.SelectedItems.OfType<ListViewItem>().Select<ListViewItem, cYo.Projects.ComicRack.Plugins.Command>((Func<ListViewItem, cYo.Projects.ComicRack.Plugins.Command>) (lvi => lvi.Tag as cYo.Projects.ComicRack.Plugins.Command)).FirstOrDefault<cYo.Projects.ComicRack.Plugins.Command>();
    ...
    {
      if (!(this.btConfigScript.Tag is cYo.Projects.ComicRack.Plugins.Command tag))
        return;
    ...
    private void btOpenFolder_Click(object sender, EventArgs e) => Program.ShowExplorer(this.lbPaths.SelectedItem as string);
    ...
    private void btResetTwitter_Click(object sender, EventArgs e) => Program.Settings.ResetTwitter();
    ...
    private void tbSaturation_DoubleClick(object sender, EventArgs e) => this.tbSaturation.Value = 0;

    private void tbBrightness_DoubleClick(object sender, EventArgs e) => this.tbBrightness.Value = 0;

    private void tbContrast_DoubleClick(object sender, EventArgs e) => this.tbContrast.Value = 0;

    private void tbSharpening_DoubleClick(object sender, EventArgs e) => this.tbSharpening.Value = 0;

    private void tbGamma_DoubleClick(object sender, EventArgs e) => this.tbGamma.Value = 0;

    private void btReset_Click(object sender, EventArgs e) => this.tbSaturation.Value = this.tbBrightness.Value = this.tbContrast.Value = this.tbGamma.Value = 0;

    private void tbOverlayScalingChanged(object sender, EventArgs e) => this.toolTip.SetToolTip((Control) this.tbOverlayScaling, string.Format("{0}%", (object) this.tbOverlayScaling.Value));
    ...
    private void btTranslate_Click(object sender, EventArgs e) => Program.StartDocument("http://comicrack.cyolito.com/faqs/12-how-to-create-language-packs");
    ...
    private void lvPackages_DragOver(object sender, DragEventArgs e) => e.Effect = e.Data.GetDataPresent(DataFormats.FileDrop) ? DragDropEffects.Copy : DragDropEffects.None;

    private void keyboardShortcutEditor_DragOver(object sender, DragEventArgs e) => e.Effect = e.Data.GetDataPresent(DataFormats.FileDrop) ? DragDropEffects.Copy : DragDropEffects.None;
    ...
    private void chkHideSampleScripts_CheckedChanged(object sender, EventArgs e) => this.FillScriptsList();
    ...
    private void FillScriptsList()
    {
      ...
      try
      {
        ...
        if (this.pluginEngine == null)
          return;
        foreach (cYo.Projects.ComicRack.Plugins.Command allCommand in this.pluginEngine.GetAllCommands())
        {
    ...
    private void SetScanButtonText() => this.btScan.Text = Program.Scanner.IsScanning ? LocalizeUtility.GetText((Control) this, "Stop", "Stop") : LocalizeUtility.GetText((Control) this, "Scan", "Scan");
