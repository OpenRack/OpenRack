...
namespace cYo.Projects.ComicRack.Viewer.Remote
{
  ...
  public class CrashReport : SoapHttpClientProtocol
  {
    ...
    public string SubmitReport(string appication, string report) => (string) this.Invoke(nameof (SubmitReport), new object[2]
    {
      (object) appication,
      (object) report
    })[0];
    ...
    public string EndSubmitReport(IAsyncResult asyncResult) => (string) this.EndInvoke(asyncResult)[0];

    public void SubmitReportAsync(string appication, string report) => this.SubmitReportAsync(appication, report, (object) null);
