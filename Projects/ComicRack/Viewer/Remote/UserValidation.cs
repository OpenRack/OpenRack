...
namespace cYo.Projects.ComicRack.Viewer.Remote
{
  ...
  public class UserValidation : SoapHttpClientProtocol
  {
    ...
    public UserValidation() => this.Url = "http://comicrack.cyolito.com/services/UserValidation.php";
    ...
    public bool HasDonated(string user) => (bool) this.Invoke(nameof (HasDonated), new object[1]
    {
      (object) user
    })[0];

    public IAsyncResult BeginHasDonated(string user, AsyncCallback callback, object asyncState) => this.BeginInvoke("HasDonated", new object[1]
    {
      (object) user
    }, callback, asyncState);
