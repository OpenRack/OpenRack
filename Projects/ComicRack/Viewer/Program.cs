...
namespace cYo.Projects.ComicRack.Viewer
{
  public static class Program
  {
    ...
    public static bool UseLocalSettings => IniFile.Default.GetValue<bool>(nameof (UseLocalSettings));
    ...
    public static IEnumerable<string> CommandLineFiles => Program.ExtendedSettings.Files ?? Enumerable.Empty<string>();
    ...
    public static void RefreshAllWindows() => Program.ForAllForms((Action<Form>) (f => f.Refresh()));
    ...
    public static IEnumerable<ComicBookValueMatcher> GetAvailableComicBookMatchers() => ComicBookValueMatcher.GetAvailableMatchers().OfType<ComicBookValueMatcher>();

    public static IEnumerable<ComicBookValueMatcher> GetUsedComicBookMatchers(int minUsage) => (IEnumerable<ComicBookValueMatcher>) Program.Database.ComicLists.GetItems<ComicSmartListItem>().SelectMany<ComicSmartListItem, ComicBookValueMatcher>((Func<ComicSmartListItem, IEnumerable<ComicBookValueMatcher>>) (n => n.Matchers.Recurse<ComicBookValueMatcher>((Func<object, IEnumerable>) (o => !(o is ComicBookGroupMatcher) ? (IEnumerable) null : (IEnumerable) ((ComicBookGroupMatcher) o).Matchers)))).Select<ComicBookValueMatcher, System.Type>((Func<ComicBookValueMatcher, System.Type>) (n => n.GetType())).GroupBy<System.Type, System.Type>((Func<System.Type, System.Type>) (n => n)).Where<IGrouping<System.Type, System.Type>>((Func<IGrouping<System.Type, System.Type>, bool>) (g => g.Count<System.Type>() >= minUsage)).Select<IGrouping<System.Type, System.Type>, ComicBookValueMatcher>((Func<IGrouping<System.Type, System.Type>, ComicBookValueMatcher>) (t => Activator.CreateInstance(t.First<System.Type>()) as ComicBookValueMatcher)).OrderBy<ComicBookValueMatcher, string>((Func<ComicBookValueMatcher, string>) (n => n.Description));
      ...
      int minUsage = 20)
    {
      ...
      System.Type[] array = Program.GetUsedComicBookMatchers(5).Select<ComicBookValueMatcher, System.Type>((Func<ComicBookValueMatcher, System.Type>) (m => m.GetType())).ToArray<System.Type>();
      foreach (ComicBookValueMatcher comicBookMatcher in Program.GetAvailableComicBookMatchers())
      {
        ...
        contextMenuBuilder.Add(comicBookMatcher.Description, false, false, (EventHandler) ((_, __) => action(m)), (object) null, ((IEnumerable<System.Type>) array).Contains<System.Type>(comicBookMatcher.GetType()) ? DateTime.MaxValue : DateTime.MinValue);
      }
    ...
    public static string[] LoadDefaultBackgroundTextures() => FileUtility.GetFiles(IniFile.GetDefaultLocations("Resources\\Textures\\Backgrounds"), SearchOption.AllDirectories, ".jpg", ".gif", ".png").OrderBy<string, string>((Func<string, string>) (s => s)).ToArray<string>();

    public static string[] LoadDefaultPaperTextures() => FileUtility.GetFiles(IniFile.GetDefaultLocations("Resources\\Textures\\Papers"), SearchOption.AllDirectories, ".jpg", ".gif", ".png").OrderBy<string, string>((Func<string, string>) (s => s)).ToArray<string>();
    ...
    public static IEnumerable<string> GetFavoritePaths() => Program.Settings.FavoriteFolders.Concat<string>(Program.Database.WatchFolders.Select<WatchFolder, string>((Func<WatchFolder, string>) (wf => wf.Folder))).Distinct<string>();
    ...
    private static void CallMainForm(string actionName, Action action) => ThreadUtility.RunInBackground(actionName, (ThreadStart) (() =>
    {
    ...
    private static void IgnoredCoverImagesChanged(object sender, EventArgs e) => ComicInfo.CoverKeyFilter = Program.Settings.IgnoredCoverImages;
    ...
    private static void StartNew(string[] args)
    {
      ...
      Program.MainForm.FormClosing += (FormClosingEventHandler) ((s, e) =>
      {
        ...
        foreach (cYo.Projects.ComicRack.Plugins.Command command in ScriptUtility.GetCommands("Shutdown"))
        {
          try
          {
    ...
    private static IEnumerable<string> SplitIconKeys(string value) => (IEnumerable<string>) value.Split(',', '#');
    ...
    private static bool InitializeDatabase(int startPercent, string readDbMessage) => Program.DatabaseManager.Open(Program.Paths.DatabasePath, Program.ExtendedSettings.DataSource, Program.ExtendedSettings.DoNotLoadQueryCaches, string.IsNullOrEmpty(readDbMessage) ? (Action<int>) null : (Action<int>) (percent => Program.StartupProgress(readDbMessage, startPercent + percent / 5)));

    private static void MainFormFormClosed(object sender, FormClosedEventArgs e) => AutomaticProgressDialog.Process((Form) null, TR.Messages["ClosingComicRack", "Closing ComicRack"], TR.Messages["SaveAllData", "Saving all Data to Disk..."], 1500, new Action(Program.CleanUp), AutomaticProgressDialogOptions.None);
    ...
    private static void StartLast(string[] args) => ControlExtensions.BeginInvoke(Program.MainForm, (Action) (() =>
    {
