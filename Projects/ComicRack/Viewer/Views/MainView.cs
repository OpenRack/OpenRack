...
namespace cYo.Projects.ComicRack.Viewer.Views
{
  public class MainView : SubView, IDisplayWorkspace, IListDisplays
  {
    ...
    public IEnumerable<TabBar.TabBarItem> GetFileTabs(bool withPlus = false) => (IEnumerable<TabBar.TabBarItem>) this.tabStrip.Items.Where<TabBar.TabBarItem>((Func<TabBar.TabBarItem, bool>) (tbi => tbi.Tag is int && (int) tbi.Tag >= 0 | withPlus)).ToArray<TabBar.TabBarItem>();
    ...
    public IEnumerable<ComicLibrary> GetLibraries(bool includeRemote = true, bool onlyLocalRemote = false) => this.tabStrip.Items.Select<TabBar.TabBarItem, object>((Func<TabBar.TabBarItem, object>) (tb => tb.Tag)).OfType<ComicExplorerView>().Select(cev => new
    {
    ...
    private void ComicBrowserForm_Disposed(object sender, EventArgs e) => this.openBrowsers.Remove(sender as ComicBrowserForm);
    ...
    private bool ShowView(string name) => this.ShowView(this.tabStrip.Items[name] ?? this.tsbLibrary);
    ...
    private static void SetBackgroundImage(ComicExplorerView view, Image img) => view.ComicBrowser.ListBackgroundImage = img;
    ...
    public bool IsComicVisible => this.tabStrip.SelectedTab != null && this.tabStrip.SelectedTab.Tag is int;
