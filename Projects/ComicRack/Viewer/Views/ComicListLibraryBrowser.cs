...
namespace cYo.Projects.ComicRack.Viewer.Views
{
    ...
    IBrowseHistory
  {
    ...
    public ComicsEditModes ComicEditMode => this.Library != null ? this.Library.EditMode : ComicsEditModes.Default;
    ...
    private TreeNode FindItemNode(IComicBookListProvider item) => item == null ? (TreeNode) null : this.FindItemNode(item.Id);
    ...
    private bool EditSmartListItem(TreeNode tn, ComicSmartListItem csli)
    {
      ...
      while (comicSmartListItem != null)
      {
        using (Form form = flag ? (Form) new SmartListQueryDialog() : (Form) new SmartListDialog())
        {
          ...
          Func<TreeNode> getNext = (Func<TreeNode>) (() => tnc.OfType<TreeNode>().SkipWhile<TreeNode>(closure_0 ?? (closure_0 = (Func<TreeNode, bool>) (n => n != tn))).Skip<TreeNode>(1).FirstOrDefault<TreeNode>((Func<TreeNode, bool>) (n => n.Tag is ComicSmartListItem)));
          Func<TreeNode> getPrev = (Func<TreeNode>) (() => tnc.OfType<TreeNode>().Reverse<TreeNode>().SkipWhile<TreeNode>(closure_1 ?? (closure_1 = (Func<TreeNode, bool>) (n => n != tn))).Skip<TreeNode>(1).FirstOrDefault<TreeNode>((Func<TreeNode, bool>) (n => n.Tag is ComicSmartListItem)));
    ...
    private ComicListItem GetCurrentNodeComicList() => this.tvQueries.SelectedNode != null ? (ComicListItem) this.tvQueries.SelectedNode.Tag : (ComicListItem) null;

    private ComicListItemCollection GetCurrentNodeComicListCollection() => this.GetNodeComicListCollection(this.tvQueries.SelectedNode);
    ...
    private void tvQueries_BeforeLabelEdit(object sender, NodeLabelEditEventArgs e) => e.CancelEdit = !this.ComicEditMode.CanEditList();

    private void tvQueries_AfterLabelEdit(object sender, NodeLabelEditEventArgs e)
    {
      ComicListItem comicListItem = e.Node == null ? (ComicListItem) null : e.Node.Tag as ComicListItem;
    ...
    private void treeContextMenu_Opening(object sender, CancelEventArgs e)
    {
      ...
      else
      {
        ...
        foreach (DeviceSyncSettings device in Program.Settings.Devices)
        {
          ...
          {
            Checked = device.Lists.FirstOrDefault<DeviceSyncSettings.SharedList>(closure_0 ?? (closure_0 = (Func<DeviceSyncSettings.SharedList, bool>) (l => l.ListId == cli.Id))) != null
          };
    ...
    private void favView_Resize(object sender, EventArgs e) => this.favView.ItemTileSize = new System.Drawing.Size(this.favView.ClientRectangle.Width - SystemInformation.VerticalScrollBarWidth - 8, FormUtility.ScaleDpiY(50));
    ...
    private void SetDropEffects(DragEventArgs e)
    {
      ...
      {
        IEditableComicBookListProvider bookListProvider = node == null ? (IEditableComicBookListProvider) null : node.Tag as IEditableComicBookListProvider;
        if (bookListProvider != null && bookListProvider.IsLibrary)
          e.Effect = e.AllowedEffect;
        this.treeSkin.SeparatorDropNodeStyle = false;
      }
    ...
    private void tvQueries_DragDrop(object sender, DragEventArgs e)
    {
      ...
      {
        IEditableComicBookListProvider list = dropNode == null ? (IEditableComicBookListProvider) null : dropNode.Tag as IEditableComicBookListProvider;
        ComicSmartListItem comicSmartListItem = dropNode == null ? (ComicSmartListItem) null : dropNode.Tag as ComicSmartListItem;
        ComicListItemFolder comicListItemFolder = dropNode == null ? (ComicListItemFolder) null : dropNode.Tag as ComicListItemFolder;
        if (list != null)
          ComicListLibraryBrowser.InsertBooksIntoToList(list, -1, dragBookContainer);
        else if (comicSmartListItem != null)
        {
          comicSmartListItem.Matchers.AddRange(dragBookContainer.CreateSeriesGroupMatchers());
        ...
        else
        {
          ...
          if (comicListItemFolder == null)
            ...
            comicListItemFolder.Items.Add(comicListItem);
        }
      }
    ...
    public void SetWorkspace(DisplayWorkspace ws) => this.quickSearch.AutoCompleteList.AddRange(Program.Settings.LibraryQuickSearchList.ToArray());
    ...
    public ComicListItem ImportList(string file) => this.ImportList((ComicListItemCollection) null, file);
    ...
    public void BrowsePrevious()
    {
      while (this.history.CanMoveCursorPrevious)
      {
        ...
        TreeNode treeNode = linkedListNode != null ? this.FindItemNode(linkedListNode.Value) : (TreeNode) null;
        if (treeNode != null)
        {
          this.tvQueries.SelectedNode = treeNode;
          this.UpdateBookList();
          break;
        }
      }
    ...
    private void library_ListCachesUpdated(object sender, EventArgs e) => this.QueueListCacheChange();
