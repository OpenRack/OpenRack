...
namespace cYo.Projects.ComicRack.Viewer.Views
{
  public class ComicListFolderFilesBrowser : ComicListFilesBrowser, IDisplayWorkspace
  {
    ...
    {
      get => this.tvFolders == null || this.tvFolders.Nodes.Count <= 0 ? this.cachedCurrentFolder : this.tvFolders.GetSelectedNodePath();
      set
      {
    ...
    protected virtual void OnInitalDisplay() => ControlExtensions.BeginInvoke(this, (Action) (() =>
    {
      this.tvFolders.Init();
      this.Update();
    ...
    private void tvFolders_AfterSelect(object sender, TreeViewEventArgs e) => this.FillBooks(this.CurrentFolder);

    private void paths_Changed(object sender, SmartListChangedEventArgs<string> e) => this.FillFavorites();
    ...
    {
      ItemViewItem itemViewItem = this.favView.FocusedItem == null ? (ItemViewItem) null : this.favView.FocusedItem as ItemViewItem;
      if (itemViewItem == null || !Program.AskQuestion((IWin32Window) this, TR.Messages["AskRemoveFavorite", "Do you really want to remove this Favorite Folder link?"], TR.Messages["Remove", "Remove"], HiddenMessageBoxes.RemoveFavorite))
        return;
      Program.Settings.FavoriteFolders.Remove(itemViewItem.Tag as string);
      this.favView.Items.Remove((IViewableItem) itemViewItem);
    }
