...
namespace cYo.Projects.ComicRack.Viewer.Controls
{
  public class PageViewItemGrouper<T> : IGrouper<IViewableItem> where T : IGrouper<ComicPageInfo>, new()
  {
    ...
    public IGroupInfo GetGroup(IViewableItem item) => this.grouper.GetGroup(((PageViewItem) item).PageInfo);

    public IEnumerable<IGroupInfo> GetGroups(IViewableItem item) => this.grouper.GetGroups(((PageViewItem) item).PageInfo);
  }
}
