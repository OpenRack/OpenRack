...
namespace cYo.Projects.ComicRack.Viewer.Controls
{
  public class PageViewItemComparer<T> : Comparer<PageViewItem>, IComparer<IViewableItem> where T : IComparer<ComicPageInfo>, new()
  {
    ...
    int IComparer<IViewableItem>.Compare(IViewableItem x, IViewableItem y) => this.Compare((PageViewItem) x, (PageViewItem) y);

    public override int Compare(PageViewItem x, PageViewItem y) => this.comparer.Compare(x.Book.Comic.GetPage(x.Page), y.Book.Comic.GetPage(y.Page));
  }
}
