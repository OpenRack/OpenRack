...
namespace cYo.Projects.ComicRack.Viewer.Controls
{
  public class PagesView : UserControl, IEditBookmark, IEditPage
  {
    ...
    public IEnumerable<ComicPageInfo> GetSelectedPages() => this.itemView.SelectedItems.Cast<PageViewItem>().Select<PageViewItem, ComicPageInfo>((Func<PageViewItem, ComicPageInfo>) (pvi => pvi.PageInfo));
    ...
    private bool IsPageSorted() => this.itemView.SortColumn != null && this.itemView.SortColumn.Id == 0 && this.itemView.ItemSortOrder == SortOrder.Ascending && this.itemView.GroupColumn == null;
    ...
    private void itemView_DragEnter(object sender, DragEventArgs e) => e.Effect = this.dragPages == null || !this.IsPageSorted() ? DragDropEffects.None : DragDropEffects.Move;

    private void itemView_DragLeave(object sender, EventArgs e) => this.itemView.MarkerVisible = false;
    ...
    public bool CanBookmark => this.Book != null && this.Book.Comic != null && this.Book.Comic.EditMode.CanEditPages() && this.itemView.SelectedCount == 1;
    ...
    {
      get => !this.CanBookmark ? (string) null : this.GetSelectedPages().First<ComicPageInfo>().Bookmark;
      set
      {
        if (!this.CanBookmark)
          return;
    ...
    private bool HasValidPages => this.Book != null && this.Book.Comic != null && this.Book.Comic.EditMode.CanEditPages() && this.itemView.SelectedCount > 0;

    bool IEditPage.IsValid => this.HasValidPages;

    ComicPageType IEditPage.PageType
    {
      get => !this.HasValidPages ? ComicPageType.Other : this.itemView.SelectedItems.OfType<PageViewItem>().Select<PageViewItem, ComicPageInfo>((Func<PageViewItem, ComicPageInfo>) (pvi => pvi.PageInfo)).FirstOrDefault<ComicPageInfo>().PageType;
      set
      {
    ...
    {
      get => !this.HasValidPages ? ImageRotation.None : this.itemView.SelectedItems.OfType<PageViewItem>().Select<PageViewItem, ComicPageInfo>((Func<PageViewItem, ComicPageInfo>) (pvi => pvi.PageInfo)).FirstOrDefault<ComicPageInfo>().Rotation;
      set
      {
        if (!this.HasValidPages)
          return;
