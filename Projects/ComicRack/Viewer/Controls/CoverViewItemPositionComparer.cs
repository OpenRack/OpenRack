...
namespace cYo.Projects.ComicRack.Viewer.Controls
{
  ...
  {
    public override int Compare(CoverViewItem x, CoverViewItem y) => x.Position.CompareTo(y.Position);

    int IComparer<IViewableItem>.Compare(IViewableItem x, IViewableItem y) => this.Compare((CoverViewItem) x, (CoverViewItem) y);
  }
}
