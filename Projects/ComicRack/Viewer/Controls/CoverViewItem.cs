...
namespace cYo.Projects.ComicRack.Viewer.Controls
{
  public class CoverViewItem : ThumbnailViewItem, IViewableItemHitTest, ISetCustomThumbnail
  {
    ...
    public int StackReadPercent
    {
      get
      {
        ...
        if (this.stackReadPercent < 0)
        {
          ...
          this.stackReadPercent = Enumerable.OfType<CoverViewItem>(this.View.GetStackItems((IViewableItem) this)).Count<CoverViewItem>((Func<CoverViewItem, bool>) (cvi => cvi.Comic.HasBeenRead)) * 100 / stackCount;
        }
    ...
    public float AverageRating
    {
      get
      {
        ...
        if ((double) this.averageRating < 0.0)
        {
          try
          {
            this.averageRating = Enumerable.OfType<CoverViewItem>(this.View.GetStackItems((IViewableItem) this)).Select<CoverViewItem, float>((Func<CoverViewItem, float>) (cvi => cvi.Comic.Rating)).Where<float>((Func<float, bool>) (r => (double) r > 0.0)).Average<float>((Func<float, float>) (r => r));
          }
          catch (Exception ex)
          {
    ...
    public float AverageCommunityRating
    {
      get
      {
        ...
        if ((double) this.averageCommunityRating < 0.0)
        {
          try
          {
            this.averageCommunityRating = Enumerable.OfType<CoverViewItem>(this.View.GetStackItems((IViewableItem) this)).Select<CoverViewItem, float>((Func<CoverViewItem, float>) (cvi => cvi.Comic.CommunityRating)).Where<float>((Func<float, bool>) (r => (double) r > 0.0)).Average<float>((Func<float, float>) (r => r));
          }
          catch (Exception ex)
          {
    ...
    public override ItemViewStates GetOwnerDrawnStates(ItemViewMode mode) => mode != ItemViewMode.Detail ? ItemViewStates.All : base.GetOwnerDrawnStates(mode);

    public override ThumbnailKey ThumbnailKey => string.IsNullOrEmpty(this.customThumbnailKey) || !this.View.IsStack((IViewableItem) this) ? this.Comic.GetFrontCoverThumbnailKey() : this.Comic.GetThumbnailKey(0, this.customThumbnailKey);
    ...
    private ComicBook CreateStackInfo()
    {
      ...
      bool flag = true;
      foreach (ComicBook ci in Enumerable.OfType<CoverViewItem>(this.View.GetStackItems((IViewableItem) this)).Select<CoverViewItem, ComicBook>((Func<CoverViewItem, ComicBook>) (ci => ci.Comic)))
      {
        if (flag)
        {
    ...
    private void ClickCoverButton(Rectangle bounds, System.Drawing.Point pt) => this.Comic.PreferredFrontCover = Numeric.Rollover(this.Comic.PreferredFrontCover, this.Comic.FrontCoverCount, pt.X >= bounds.Width / 2 ? 1 : -1);
    ...
    public ComicBookSeriesStatistics SeriesStats => this.StatsProvider == null ? (ComicBookSeriesStatistics) null : this.StatsProvider.GetSeriesStats(this.Comic);

    public bool Contains(System.Drawing.Point pt) => !this.drawnRect.HasValue || this.drawnRect.Value.Contains(pt);

    public bool IntersectsWith(Rectangle rc) => !this.drawnRect.HasValue || this.drawnRect.Value.IntersectsWith(rc);
