...
namespace cYo.Projects.ComicRack.Viewer.Controls
{
  public class CoverViewItemStatsGrouper<T> : IGrouper<IViewableItem> where T : IGrouper<ComicBookSeriesStatistics>, new()
  {
    ...
    public IGroupInfo GetGroup(IViewableItem item) => this.statsGrouper.GetGroup(((CoverViewItem) item).SeriesStats);

    public bool IsMultiGroup => this.statsGrouper.IsMultiGroup;

    public IEnumerable<IGroupInfo> GetGroups(IViewableItem item) => this.statsGrouper.GetGroups(((CoverViewItem) item).SeriesStats);
  }
}
