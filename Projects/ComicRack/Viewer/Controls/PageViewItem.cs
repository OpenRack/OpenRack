...
namespace cYo.Projects.ComicRack.Viewer.Controls
{
  public class PageViewItem : ThumbnailViewItem
  {
    ...
    public void SetPageType(ComicPageType pageType) => this.book.Comic.UpdatePageType(this.Page, pageType);

    public void SetPageRotation(ImageRotation rotation) => this.book.Comic.UpdatePageRotation(this.Page, rotation);

    public void SetPagePosition(ComicPagePosition position) => this.book.Comic.UpdatePagePosition(this.Page, position);
    ...
    public override ItemViewStates GetOwnerDrawnStates(ItemViewMode displayType) => displayType == ItemViewMode.Tile ? ItemViewStates.Selected | ItemViewStates.Hot : base.GetOwnerDrawnStates(displayType);
    ...
    public override void OnDraw(ItemDrawInformation drawInfo)
    {
      ...
      ComicListField comicListField = drawInfo.Header != null ? drawInfo.Header.Tag as ComicListField : (ComicListField) null;
      ...
      {
        using (IItemLock<ThumbnailImage> itemLock = comicListField == null || comicListField.DisplayProperty == "Thumbnail" ? this.GetThumbnail(drawInfo) : (IItemLock<ThumbnailImage>) null)
        {
          ...
          if (itemLock != null)
          {
            ...
            int page = this.Page;
            size = itemLock.Item.OriginalSize;
          ...
          Image thumbnail = (Image) itemLock?.Item.GetThumbnail(height1);
          ...
          switch (drawInfo.DisplayType)
          {
            case ItemViewMode.Thumbnail:
              this.Animate(thumbnail);
              ThumbRenderer thumbRenderer = new ThumbRenderer(thumbnail, flags | ThumbnailDrawingOptions.EnablePageNumber)
              {
            ...
            case ItemViewMode.Tile:
              this.Animate(thumbnail);
              ThumbTileRenderer thumbTileRenderer1 = new ThumbTileRenderer(thumbnail, flags);
            ...
            case ItemViewMode.Detail:
              ...
              {
                if (comicListField.DisplayProperty == "Thumbnail")
                {
                  this.Animate(thumbnail);
                  if (thumbnail == null)
                    break;
                  bounds.Height = bounds.Width * thumbnail.Height / thumbnail.Width;
                  bounds.Inflate(-2, -2);
                  drawInfo.Graphics.IntersectClip(bounds);
                  if (this.PageInfo.IsBookmark)
                    flags |= ThumbnailDrawingOptions.Bookmarked;
                  ThumbRenderer.DrawThumbnail(drawInfo.Graphics, thumbnail, bounds, flags, (ComicBook) null, this.Opacity);
                  break;
                }
                ...
                string stringValue = this.GetStringValue(comicListField.DisplayProperty);
                bounds.Inflate(-2, 0);
