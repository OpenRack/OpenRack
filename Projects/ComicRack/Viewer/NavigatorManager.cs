  ...
  public class NavigatorManager : IOpenBooksManager
  {
    ...
    public int CurrentSlot
    {
      ...
      set
      {
        ...
        try
        {
          ComicBookNavigator comicBookNavigator = this.currentSlot < 0 ? (ComicBookNavigator) null : this.slots[this.currentSlot];
          ...
          this.comicDisplay.Book = comicBookNavigator;
        }
        catch
        {
    ...
    public int OpenCount => this.slots.Count<ComicBookNavigator>((Func<ComicBookNavigator, bool>) (nav => nav != null));

    public IEnumerable<string> OpenFiles => this.slots.Where<ComicBookNavigator>((Func<ComicBookNavigator, bool>) (nav => nav != null && nav.Comic != null && nav.Comic.EditMode.IsLocalComic())).Select<ComicBookNavigator, string>((Func<ComicBookNavigator, string>) (nav => nav.Comic.FilePath));

    public bool IsOpen(ComicBook cb) => this.slots.Any<ComicBookNavigator>((Func<ComicBookNavigator, bool>) (nav => nav != null && nav.Comic == cb));
    ...
    public bool Open(ComicBook cb, bool inNewSlot, int page = 0) => this.Open(cb, (OpenComicOptions) (0 | (inNewSlot ? 32 : 0)), page);

    public bool Open(string file, OpenComicOptions options = OpenComicOptions.None, int page = 0) => this.Open(Program.BookFactory.Create(file, Program.Settings.AddToLibraryOnOpen ? CreateBookOption.AddToStorage : CreateBookOption.AddToTemporary), options, page);

    public bool Open(string file, bool inNewSlot, int page = 0) => this.Open(file, (OpenComicOptions) (0 | (inNewSlot ? 32 : 0)), page);
    ...
    private void Comic_PropertyChanged(object sender, PropertyChangedEventArgs e) => this.OnOpenComicsChanged();
