  ...
  public class MainForm : Form, IMain, IContainerControl, IPluginConfig, IApplication, IBrowser
  {
    ...
    public MainForm()
    {
      ...
      if (ScriptUtility.Initialize((IWin32Window) this, (IApplication) this, (IBrowser) this, (IComicDisplay) this.ComicDisplay, (IPluginConfig) this, (IOpenBooksManager) this.OpenBooks))
      {
        ...
        foreach (cYo.Projects.ComicRack.Plugins.Command command in ScriptUtility.Scripts.GetCommands("DrawThumbnailOverlay"))
        {
          cYo.Projects.ComicRack.Plugins.Command sc = command;
          sc.PreCompile();
    ...
    private void ComicDisplay_PageChanged(object sender, BookPageEventArgs e) => Win7.InvalidateThumbnail(this.OpenBooks.CurrentBook.Comic.FilePath);
    ...
    private void BookDragDrop(object sender, DragEventArgs e) => this.OpenSupportedFile(((string[]) e.Data.GetData(DataFormats.FileDrop))[0]);
    ...
    public ComicListItem ImportComicList(string file) => this.FindFirstService<IImportComicList>()?.ImportList(file);
    ...
    public void StartFullScan() => Program.QueueManager.StartScan(true, Program.Settings.RemoveMissingFilesOnFullScan);

    public void UpdateComics() => Program.Database.Books.Concat<ComicBook>((IEnumerable<ComicBook>) Program.BookFactory.TemporaryBooks).ForEach<ComicBook>((Action<ComicBook>) (cb => Program.QueueManager.AddBookToFileUpdate(cb, true)));

    public void UpdateWebComics(bool refresh = false) => Program.Database.Books.Concat<ComicBook>((IEnumerable<ComicBook>) Program.BookFactory.TemporaryBooks).ForEach<ComicBook>((Action<ComicBook>) (cb => this.UpdateWebComic(cb, refresh)));

    public void UpdateWebComics() => this.UpdateWebComics(false);

    public void UpdateWebComic(ComicBook cb, bool refresh) => Program.QueueManager.AddBookToDynamicUpdate(cb, refresh);
    ...
    public IEditPage GetPageEditor() => (IEditPage) new MainForm.PageEditorWrapper(FormUtility.FindActiveService<IEditPage>());

    private MainForm.BookmarkEditorWrapper GetBookmarkEditor() => new MainForm.BookmarkEditorWrapper(FormUtility.FindActiveService<IEditBookmark>());
    ...
    private bool RemoveBookmarkAvailable() => !string.IsNullOrEmpty(this.GetBookmarkEditor().Bookmark);
    ...
    private static string AddExtension(string file, string ext) => !Path.HasExtension(file) ? file + ext : file;
    ...
    public void ShowPortableDevices(DeviceSyncSettings dss = null, Guid? guid = null) => DevicesEditDialog.Show((IWin32Window) (Form.ActiveForm ?? (Form) this), (IList<DeviceSyncSettings>) Program.Settings.Devices, dss, guid);
    ...
    private void UpdateListConfigMenus() => this.UpdateListConfigMenus(this.miListLayouts.DropDownItems);

    private void OnOpenRecent(object sender, EventArgs e) => this.OpenSupportedFile(this.recentFiles[Convert.ToInt32(((ToolStripItem) sender).Text.Substring(0, 2)) - 1], Program.Settings.OpenInNewTab);
    ...
    private void ReaderFormKeyDown(object sender, KeyEventArgs e) => e.Handled = this.commands.InvokeKey(e.KeyData);
    ...
    private void btn_CloseClick(object sender, EventArgs e) => this.OpenBooks.Close((int) ((TabBar.TabBarItem) sender).Tag);
    ...
    private void OpenBooks_Clicked(object sender, EventArgs e) => this.OpenBooks.CurrentSlot = (int) (sender is ToolStripItem ? ((ToolStripItem) sender).Tag : ((TabBar.TabBarItem) sender).Tag);

    private void OpenBooks_Selected(object sender, CancelEventArgs e) => this.OpenBooks.CurrentSlot = (int) (sender is ToolStripItem ? ((ToolStripItem) sender).Tag : ((TabBar.TabBarItem) sender).Tag);
    ...
    private void tsCurrentPage_Click(object sender, EventArgs e) => Program.Settings.TrackCurrentPage = !Program.Settings.TrackCurrentPage;

    private void tabContextMenu_Opening(object sender, CancelEventArgs e) => this.sepBeforeRevealInBrowser.Visible = this.cmRevealInExplorer.Visible = this.cmCopyPath.Visible = this.ComicDisplay == null || this.ComicDisplay.Book == null || this.ComicDisplay.Book.Comic.EditMode.IsLocalComic();
    ...
    private void backgroundSaveTimer_Tick(object sender, EventArgs e) => Program.DatabaseManager.SaveInBackground();
    ...
    private void fileMenu_DropDownOpening(object sender, EventArgs e) => this.miUpdateAllComicFiles.Visible = !Program.Settings.AutoUpdateComicsFiles;
    ...
    private void TrackBar_Scroll(object sender, EventArgs e) => this.FindActiveService<IItemSize>()?.SetItemSize(this.thumbSize.TrackBar.Value);
    ...
    private void tbBookmarks_DropDownOpening(object sender, EventArgs e) => this.UpdateBookmarkMenu(this.tbBookmarks.DropDownItems, 0);

    private void cmBookmarks_DropDownOpening(object sender, EventArgs e) => this.UpdateBookmarkMenu(this.cmBookmarks.DropDownItems, 0);

    private void tbPrevPage_DropDownOpening(object sender, EventArgs e) => this.UpdateBookmarkMenu(this.tbPrevPage.DropDownItems, -1);

    private void tbNextPage_DropDownOpening(object sender, EventArgs e) => this.UpdateBookmarkMenu(this.tbNextPage.DropDownItems, 1);

    private void miBookmarks_DropDownOpening(object sender, EventArgs e) => this.UpdateBookmarkMenu(this.miBookmarks.DropDownItems, 0);
    ...
    private void NotifyIconMouseDoubleClick(object sender, MouseEventArgs e) => this.RestoreFromTray();
    ...
    private void pageContextMenu_Closed(object sender, ToolStripDropDownClosedEventArgs e) => this.StartMouseDisabledTimer();

    private void showDisableTimer_Tick(object sender, EventArgs e) => this.ComicDisplay.MouseClickEnabled = true;

    public Control Control => (Control) this;

    public bool IsComicVisible => this.ReaderUndocked || this.BrowserDock != DockStyle.Fill || this.mainView.IsComicVisible;

    public bool BrowserVisible
    {
      get => !this.ReaderUndocked && this.BrowserDock != DockStyle.Fill ? this.mainViewContainer.Expanded : this.savedBrowserVisible;
      set
      {
        if (!this.ReaderUndocked && this.BrowserDock != DockStyle.Fill)
        {
    ...
    public IEnumerable<ComicBook> ReadDatabaseBooks(string file) => throw new Exception("The method or operation is not implemented.");
    ...
    public Bitmap GetComicPublisherIcon(ComicBook cb) => (ComicBook.PublisherIcons.GetImage(cb.GetPublisherIconKey()) ?? ComicBook.PublisherIcons.GetImage(cb.Publisher)).CreateCopy(true);

    public Bitmap GetComicImprintIcon(ComicBook cb) => (ComicBook.PublisherIcons.GetImage(cb.GetImprintIconKey()) ?? ComicBook.PublisherIcons.GetImage(cb.Imprint)).CreateCopy(true);

    public Bitmap GetComicAgeRatingIcon(ComicBook cb) => ComicBook.AgeRatingIcons.GetImage(cb.AgeRating).CreateCopy(true);

    public Bitmap GetComicFormatIcon(ComicBook cb) => ComicBook.FormatIcons.GetImage(cb.Format).CreateCopy(true);
    ...
    public IEnumerable<ComicBook> GetLibraryBooks() => (IEnumerable<ComicBook>) Program.Database.Books.ToArray();
    ...
    public ComicBook GetBook(string file) => Program.BookFactory.Create(file, CreateBookOption.AddToTemporary);
    ...
    public void SelectComics(IEnumerable<ComicBook> books) => this.FindActiveService<IComicBrowser>()?.SelectComics(books);
    ...
    public IEnumerable<string> LibraryPaths => (IEnumerable<string>) Program.Settings.ScriptingLibraries.Replace("\n", "").Replace("\r", "").Split(';', StringSplitOptions.RemoveEmptyEntries);
    ...
    public class BookmarkEditorWrapper : IEditBookmark
    {
      ...
      public string BookmarkProposal => !this.CanBookmark ? string.Empty : this.editor.BookmarkProposal;
